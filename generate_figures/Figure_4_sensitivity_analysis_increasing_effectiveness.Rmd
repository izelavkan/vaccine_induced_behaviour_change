---
title: "Figure_4_sensitivity"
author: "izel and ewan"
date: "2025-05-12"
...
```{r}
comix_org <- read.csv("../data/processed_contact_data/all_ages/UK_CoMix.csv")
...
psi_alpha <- psi_alpha * seq(from=1,to=0.5,by=-0.5/nrow(data_comm))
sigma_u <- c_uv/c_uu
sigma_v <- c_vv/c_uv
...
for (i in 1:length(data_comm$perc_first_dose)) {
  ...
  data_comm$a_alpha[i] <- 1 - psi_alpha[i] * sigma_u * sigma_v 
  
  data_comm$b_alpha[i] <- 2 * (1 + psi_alpha[i] * sigma_u * (sigma_v - 2 * sigma_u))
  
  data_comm$c_alpha[i] <- 1 + 2 * psi_alpha[i] * sigma_u * (sigma_v - 2 * sigma_u) + (psi_alpha[i] * sigma_u * sigma_v)^2
  ...
  data_comm$Delta_R_homo_alpha[i] <- (data_comm$contact_model_2[i]/c_uu) * (1 - (1 - psi_alpha[i]) * data_comm$perc_first_dose[i]/100)
...
psi_delta <- psi_delta * seq(from=1,to=0.5,by=-0.5/nrow(data_comm))
sigma_u <- c_uv/c_uu
sigma_v <- c_vv/c_uv
...
for (i in 1:length(data_comm$perc_first_dose)) {
  ...
  data_comm$a_delta[i] <- 1 - psi_delta[i] * sigma_u * sigma_v 
  
  data_comm$b_delta[i] <- 2 * (1 + psi_delta[i] * sigma_u * (sigma_v - 2 * sigma_u))
  
  data_comm$c_delta[i] <- 1 + 2 * psi_delta[i] * sigma_u * (sigma_v - 2 * sigma_u) + (psi_delta[i] * sigma_u * sigma_v)^2
  ...
  data_comm$Delta_R_homo_delta[i] <- (data_comm$contact_model_2[i]/1.68) * (1 - (1 - psi_delta[i]) * data_comm$perc_first_dose[i]/100)
...
psi_omicron <- psi_omicron * seq(from=1,to=0.5,by=-0.5/nrow(data_comm))
...
for (i in 1:length(data_comm$perc_first_dose)) {
  ...
  data_comm$a_omicron[i] <- 1 - psi_omicron[i] * sigma_u * sigma_v 
  
  data_comm$b_omicron[i] <- 2 * (1 + psi_omicron[i] * sigma_u * (sigma_v - 2 * sigma_u))
  
  data_comm$c_omicron[i] <- 1 + 2 * psi_omicron[i] * sigma_u * (sigma_v - 2 * sigma_u) + (psi_omicron[i] * sigma_u * sigma_v)^2
  ...
  data_comm$Delta_R_homo_omicron[i] <- (data_comm$contact_model_2[i]/c_uu) * (1 - (1 - psi_omicron[i]) * data_comm$perc_first_dose[i]/100)
...
```{r}
ggsave('Figures/Fig_4_sensitivity_increasing_effectiveness.png',
  plot = get_last_plot(),
  device = png,
  path = '.',
  width = 2250,
