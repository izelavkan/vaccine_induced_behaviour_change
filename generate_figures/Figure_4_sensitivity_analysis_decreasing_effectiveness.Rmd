...
psi_alpha <- psi_alpha * seq(from=1,to=2,by=1/nrow(data_comm))
sigma_u <- c_uv/c_uu
sigma_v <- c_vv/c_uv
...
psi_delta <- psi_delta * seq(from=1,to=2,by=1/nrow(data_comm))
sigma_u <- c_uv/c_uu
sigma_v <- c_vv/c_uv
...
psi_omicron <- psi_omicron * seq(from=1,to=2,by=1/nrow(data_comm))
...
```{r}
ggsave('Figures/Fig_4_sensitivity_decreasing_effectiveness.png',
  plot = get_last_plot(),
  device = png,
  path = '.',
  width = 2250,
