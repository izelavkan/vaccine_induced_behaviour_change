---
title: "Figure_3_sensitivity_analysis"
...
folder_path <- "../data/processed_contact_data/young_children_removed/bootstrap_samples"
...
for (dat in 1:length(date_prepared_data)) {
  ...
  for (i in 1:length(sample_dat$date)) {
    if (i < 4) {
      sample_dat$mean.12.17.to.12.17[i] <- 0
      sample_dat$mean.12.17.to.18.45[i] <- 0
      sample_dat$mean.12.17.to.45.64[i] <- 0
      sample_dat$mean.12.17.to.65.120[i] <- 0
    
      sample_dat$mean.18.45.to.12.17[i] <- sample_dat$X18.45.to.12.17[i]/sample_dat$N.18.45[i]
      ...
      sample_dat$mean.45.64.to.12.17[i] <- sample_dat$X45.64.to.12.17[i]/sample_dat$N.45.64[i]
      ...
      sample_dat$mean.65.120.to.12.17[i] <- sample_dat$X65.120.to.12.17[i]/sample_dat$N.65.120[i]
      ...
      sample_dat$mean.12.17.to.12.17[i] <- sample_dat$X12.17.to.12.17[i]/sample_dat$N.12.17[i]
      sample_dat$mean.12.17.to.18.45[i] <- sample_dat$X12.17.to.18.45[i]/sample_dat$N.12.17[i]
      sample_dat$mean.12.17.to.45.64[i] <- sample_dat$X12.17.to.45.64[i]/sample_dat$N.12.17[i]
      sample_dat$mean.12.17.to.65.120[i] <- sample_dat$X12.17.to.65.120[i]/sample_dat$N.12.17[i]
  ...
  for (i in 1:length(sample_dat$date)) {
    sample_dat$contact.12.17[i] <- sample_dat$mean.12.17.to.12.17[i] + sample_dat$mean.12.17.to.18.45[i] + sample_dat$mean.12.17.to.45.64[i] + sample_dat$mean.12.17.to.65.120[i]
  
    sample_dat$contact.18.45[i] <- sample_dat$mean.18.45.to.12.17[i] + sample_dat$mean.18.45.to.18.45[i] + sample_dat$mean.18.45.to.45.64[i] + sample_dat$mean.18.45.to.65.120[i]
  
    sample_dat$contact.45.64[i] <- sample_dat$mean.45.64.to.12.17[i] + sample_dat$mean.45.64.to.18.45[i] + sample_dat$mean.45.64.to.45.64[i] + sample_dat$mean.45.64.to.65.120[i]
  
    sample_dat$contact.65.120[i] <- sample_dat$mean.65.120.to.12.17[i] + sample_dat$mean.65.120.to.18.45[i] + sample_dat$mean.65.120.to.45.64[i] + sample_dat$mean.65.120.to.65.120[i]
  
  }

  sample_dat$"12-17" <- sample_dat$contact.12.17 
  ...
  for (i in 1:length(sample_dat$date)) {
    ...
    sample_dat$contact[i] <- (sample_dat$contact.12.17[i]*sample_dat$N.12.17[i] + sample_dat$contact.18.45[i]*sample_dat$N.18.45[i] + sample_dat$contact.45.64[i]*sample_dat$N.45.64[i] + sample_dat$contact.65.120[i]*sample_dat$N.65.120[i])/(sample_dat$N.12.17[i] +sample_dat$N.18.45[i] +sample_dat$N.45.64[i] +sample_dat$N.65.120[i])
...
vacc_contact$vacc_tenth <- combined_data[[1]]$perc_first_dose/15

vacc_contact$contact_model <- 1.67 * (1-vacc_contact$vacc/100)^2 + 3.85 * 2 * (1-vacc_contact$vacc/100) * (vacc_contact$vacc/100) + 3.7 * (vacc_contact$vacc/100)^2

vacc_contact$contact_model_lower <- 1.23 * (1-vacc_contact$vacc/100)^2 + 3.04 * 2 * (1-vacc_contact$vacc/100) * (vacc_contact$vacc/100) + 3.3 * (vacc_contact$vacc/100)^2

vacc_contact$contact_model_upper <- 2.23 * (1-vacc_contact$vacc/100)^2 + 4.52 * 2 * (1-vacc_contact$vacc/100) * (vacc_contact$vacc/100) + 4.19 * (vacc_contact$vacc/100)^2
```
...
plot_all <- ggplot() +
  ...
  scale_y_continuous(name = "Average number of contacts\n(per person)",
                     limits = c(0,7.5),
                     sec.axis = sec_axis(~. * 15, name = "Vaccinated population (%)")) +
...
```{r}
ggsave('Figures/Fig_3_sensitivity_no_young_children.png',
  plot = get_last_plot(),
  device = png,
  path = '.',
  width = 2250,
